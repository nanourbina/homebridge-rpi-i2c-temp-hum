...
class AHTx0 {
    ...
    constructor(busNumber: number, i2cAddress:number) {
        this.busNumber = busNumber;
        this.address = i2cAddress;
        this.dataBuffer = Buffer.alloc(5);

        this.i2cBus = openSync(busNumber);
    ...
    open(){
        this.i2cBus = openSync(this.busNumber);            
        this._reset();
        if (!this._calibrate())
...
class RPITemperatureAndHumidity implements AccessoryPlugin {
  ...
  private temperature = -273;
  private relHumidity = 0;
  private aht10: AHTx0;
